        ...
        setTimeout(() => { 
            if (config.timer && config.timer_type == 1) {
                ...
                utils.roundTimes.forEach(e => {
                    ChatLib.chat("&d[&bExtras&d] &7Round " + x + " Took &b" + e + "&r&7 seconds.")
                    x = x + 1
                })
            }
        }, 1000);
	...
	if (config.timer && utils.inWoolWars) {
		if (stateName == "Active" && !utils.roundActive) {
			...
			if (config.timer_type == 0) {
				ChatLib.chat("&d[&bExtras&d] &7Round " + round + " Took &b" + (Math.floor((endTime - startTime)/10)/100).toFixed(2) + "&r&7 seconds.")
			}
		}
	}
})
